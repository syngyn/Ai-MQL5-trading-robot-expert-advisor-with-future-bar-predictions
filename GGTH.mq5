...
#property version   "1.98" // Final fix for all ulong/long data type warnings
...
bool SendToDaemon(const double &features[], double current_price, double atr_val, DaemonResponse &response)
{
   ...
   // ========================= FIX IS HERE ========================= //
   // The 'start_time' and 'last_status_time' variables MUST be 'ulong' to match the
   // return type of GetTickCount() and prevent the "loss of data" warning.
   ...
   while(GetTickCount() - start_time < (ulong)RequestTimeout)
   {
      ...
      if(FileIsExist(response_path))
      {
         ...
         if(!all_prices_valid || !JsonGetValue(content, "confidence_score", temp_confidence) || !JsonGetValue(content, "buy_probability", temp_buy) || !JsonGetValue(content, "sell_probability", temp_sell))
         {
            g_last_error = "Failed to parse all values from JSON response.";
            g_connection_status = "Error";
            PrintFormat("ERROR: %s", g_last_error);
            continue;
         }
...
int OnInit()
{
   Print("=== GGTH EA v1.98 Initializing - All functions restored & errors fixed ===");
